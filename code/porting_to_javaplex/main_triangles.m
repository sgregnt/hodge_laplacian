...
stream = api.Plex4.createVietorisRipsStream(point_cloud, max_dimension, max_filtration, num_divisions);
...
incidence_1_matrix = d_1_array;
edge_list = find_edges(incidence_1_matrix);
...
converter = api.Plex4.createHomMatrixConverter(stream, stream);
d_2_array = converter.toArray(d_2);
d_2_array_incidence = transpose(d_2_array);
stream.getSize()
scatter(point_cloud(:,1), point_cloud(:,2), marker_size, 'og', 'filled');
hold on;
%draw_arrows(edges_points)
...
triangle_list = find_triangles(incidence_2_matrix, edge_list, point_cloud);
hold on;
